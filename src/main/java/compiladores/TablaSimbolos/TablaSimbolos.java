...
public class TablaSimbolos {
     ...
     * @return Id that corresponds to the name specified in the parameter. If more
     *         that one
    ...
    public Funcion buscarUltimoContexto() {
        Map<String, Id> m;

        for (int j = tabla.size() - 1; j >= 0; j--) {
            m = tabla.get(j);

            for (Map.Entry<String, Id> entry : m.entrySet()) {
                if (entry.getValue() instanceof Funcion)
                    return (Funcion) entry.getValue();
            }
        }
        return null;
    }
    ...
    public void imprimeWarnings() {
        ...
        for (Id id : con.values()) {
            ...
            if (!id.getUsado())
                System.err.println(Colors.CYAN_BOLD + "warning: " + "La " + tipo + " \'" + id.getNombre()
                        + "\' nunca se utiliza" + Colors.ANSI_RESET);
            if (!id.getInicializado())
                System.err.println(Colors.CYAN_BOLD + "warning: " + "La " + tipo + " \'" + id.getNombre()
                        + "\' no está " + fin + Colors.ANSI_RESET);
        }
    }
